<?php
	...
	if(isset($_POST['partner_id'])){
	    ...
	    $stmt = $conn->prepare("SELECT users.user_name,bookings.* from bookings LEFT JOIN users ON users.user_id = bookings.user_id WHERE (bookings.partner_id = :partner_id AND status = 3) AND DATE(created_on) = CURDATE()");
