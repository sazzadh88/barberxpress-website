<?php
	require '../config/db.php';
	   ...
	   // FROM services LEFT JOIN business_partners ON business_partners.partner_id = services.partner_id 
	   // WHERE business_partners.partner_otp_verified = 1 AND business_partners.partner_profile_verified =1
	   // ORDER BY business_partners.partner_id DESC LIMIT 20"); 
	   
	    $stmt = $conn->prepare("SELECT business_partners.partner_address,business_partners.partner_specialization,
	    business_partners.partner_mobile, business_partners.partner_display_name,business_partners.partner_category,
	    business_partners.partner_sub_category, services.* FROM business_partners 
	    LEFT JOIN services ON business_partners.partner_id = services.partner_id WHERE business_partners.partner_otp_verified = 1 
	    AND business_partners.partner_profile_verified =1 ORDER BY business_partners.partner_id DESC LIMIT 20");
	    $stmt->execute();
	    $data = [];
